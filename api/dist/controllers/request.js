...
export class RequestController {
    ...
    async getRequestDetailById(req, res) {
        try {
            const { id } = req.params;
            ...
            const userApprovalStep = res.locals.user.role === UserRole.TEACHER
                ? request.Approvals.flatMap(a => a.approvalSteps).find(step => step.userId === res.locals.user.id && step.status === ApprovalStatus.PENDING)
                : null;
