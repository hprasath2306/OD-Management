...
export class RequestService {
    ...
    async createRequest(data) {
        ...
        if (!flowTemplate)
            throw new Error(`Flow template not found for ${data.needsLab ? "lab" : "no lab"} flow`);
        if (data.needsLab && !lab?.incharge?.user)
            throw new Error("Lab in-charge not found");
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            for (const groupId of uniqueGroupIds) {
                ...
                if (!firstStep)
                    throw new Error("No first step found in flow template");
                ...
                const approvalStepData = (() => {
                    if (firstStep.role === 'LAB_INCHARGE' && data.needsLab) {
                        ...
                        if (!approver)
                            throw new Error(`No approver found for role ${firstStep.role} in group ${groupId}`);
                        return {
                            sequence: firstStep.sequence,
                            status: ApprovalStatus.PENDING,
                            approvalId: approval.id,
    ...
    async processApprovalStep(userId, data) {
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            const nextStepData = (async () => {
                if (nextFlowStep.role === 'LAB_INCHARGE' && approval.request.needsLab) {
                    if (!lab)
                        throw new Error('Lab not found for LAB_INCHARGE step');
                    return {
                    ...
                    if (!hod)
                        throw new Error(`No HOD found for department ${groupDepartmentId}`);
                    return {
                        sequence: nextFlowStep.sequence,
                        status: ApprovalStatus.PENDING,
                        approvalId: approval.id,
                    ...
                    if (!approver)
                        throw new Error(`No approver found for role ${nextFlowStep.role} in group ${approval.groupId}`);
                    return {
                        sequence: nextFlowStep.sequence,
                        status: ApprovalStatus.PENDING,
                        approvalId: approval.id,
    ...
    async getApproverRequests(userId) {
        ...
        return pendingSteps.map((step) => {
            ...
            const groupStudents = request.students.filter((rs) => rs.student.groupId === approverGroupId);
