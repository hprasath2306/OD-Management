...
export class FlowTemplateService {
  ...
  async getAllFlowTemplates(): Promise<FlowTemplate[]> {
  ...
  async getFlowTemplateById(id: string): Promise<FlowTemplate | null> {
  ...
  async getFlowTemplateByName(name: string): Promise<FlowTemplate | null> {
    return await prisma.flowTemplate.findUnique({
      where: { name },
      include: {
        steps: {
  ...
  async deleteFlowTemplate(id: string): Promise<FlowTemplate> {
    return await prisma.$transaction(async (tx) => {
