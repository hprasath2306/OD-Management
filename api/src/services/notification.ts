import { ApprovalStatus, ODCategory, RequestType, Role, UserRole } from '@prisma/client';
...
export class NotificationService {
   ...
   */
  async notifyNewRequest(requestId: string) {
    ...
    try {
      ...
      const initialApprovers = request.Approvals.flatMap(approval => 
        approval.approvalSteps.map(step => step.User)
      ).filter(Boolean);
      ...
      for (const approver of initialApprovers) {
        if (approver?.pushToken) {
          ...
          const success = await this.sendNotification(
            approver.pushToken,
            'New OD Request',
            `${request.requestedBy.name || 'A student'} submitted a new OD request that needs your approval`,
   ...
   */
  async notifyRequestStatus(requestId: string, approvalStepId: string, approve: boolean) {
    ...
    try {
      ...
      if (approve) {
        ...
        const nextStep = approval.approvalSteps.find(step => 
          step.sequence === approvalStep.sequence + 1 && step.status === ApprovalStatus.PENDING
        );
        ...
        if (nextStep?.User?.pushToken) {
          ...
          const success = await this.sendNotification(
            nextStep.User.pushToken,
            'OD Request Awaiting Approval',
            `An OD request approved by ${approvalStep.User?.name || 'a previous approver'} now requires your review`,
            { requestId, type: 'pending_approval' }
          );
        ...
        const allApproved = approval.approvalSteps.every(step => 
          step.status === ApprovalStatus.APPROVED || step.sequence > approvalStep.sequence
        );
   ...
   */
  async notifyRequestRequester(requestId: string, approved: boolean) {
    ...
    try {
      ...
      for (const user of studentUsers) {
        if (user.pushToken) {
          ...
          const success = await this.sendNotification(
            user.pushToken,
            approved ? 'OD Request Approved' : 'OD Request Rejected',
            approved 
              ? 'Your OD request has been fully approved' 
              : 'Your OD request was rejected',
            { requestId, type: approved ? 'approved' : 'rejected' }
          );
   ...
   */
  private async sendNotification(pushToken: string, title: string, body: string, data: any = {}) {
